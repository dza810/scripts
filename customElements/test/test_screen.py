...
def test_run_search_with_condition(
    db_connection_with_tables: sqlite3.Connection,
) -> None:
    con = db_connection_with_tables
    insert(
        con, "car", {"make": "Tesla", "model": "Model 3", "price": 50000, "electric": 1}
    )
    insert(
        con, "car", {"make": "Ford", "model": "Focus", "price": 30000, "electric": 0}
    )
...
def test_register_insert_and_update(
    db_connection_with_tables: sqlite3.Connection,
) -> None:
    con = db_connection_with_tables
    screen = get_screen("car_list", con)
...
def test_register_invalid_insert_key(
    db_connection_with_tables: sqlite3.Connection,
) -> None:
    con = db_connection_with_tables
    screen = get_screen("car_list", con)
...
def test_search_union_injection_no_data_leak(insert_cars: sqlite3.Connection) -> None:
    ...
    assert (
        screen.search(
            {"model": "x' UNION SELECT make, model, price, electric FROM car--"}
        )
        == []
    )
...
def test_select_injection_value(db_connection_with_tables: sqlite3.Connection) -> None:
    con = db_connection_with_tables
    insert(
        con, "car", {"make": "Tesla", "model": "Model 3", "price": 100, "electric": 1}
    )
...
def test_register_update_injection_id_safe(insert_cars: sqlite3.Connection) -> None:
    ...
    prices = [
        r["price"]
        for r in con.execute("SELECT price FROM car ORDER BY car_id").fetchall()
    ]
...
def test_second_order_injection_safe(
    db_connection_with_tables: sqlite3.Connection,
) -> None:
    con = db_connection_with_tables
    screen = get_screen("car_list", con)
    ...
    screen.run_insert(
        "car", {"make": "Tesla", "model": payload, "price": 1, "electric": 0}
    )
