...
def test_api_getSearchForms(api_client: TestClient) -> None:
    ...
    assert r.status_code == 200
    assert [f["search_form_cd"] for f in r.json()] == [
        "make",
        "model",
        "price",
        "electric",
...
def test_api_search_injection_safe(api_client: TestClient) -> None:
    ...
    for payload in ["' OR '1'='1", "' UNION SELECT 1,2,3,4--", "'; DROP TABLE car;--"]:
        r = api_client.post(
            "/search?screenCd=car_list", json={"params": {"model": payload}}
        )
        assert r.status_code == 200
        assert r.json() == []
...
def test_api_register_injection_value_safe(api_client: TestClient) -> None:
    ...
    r = api_client.post(
        ...
        json={
            "insertList": [
                {"make": "Tesla", "model": payload, "price": 50000, "electric": 1}
            ],
            "deleteList": [],
            "updateList": [],
        },
...
def test_api_injection_no_table_damage(api_client: TestClient) -> None:
    ...
    for payload in payloads:
        ...
        api_client.post(
            ...
            json={
                "insertList": [
                    {"make": "Tesla", "model": payload, "price": 1, "electric": 0}
                ],
                "deleteList": [payload],
