...
def make_table(table, screen_cd, screen_name):
    with connect() as con:
...
#             con, screen_table.name, {"screen_cd": "screen_master", "screen_name": "画面管理"}
#         ).lastrowid
#
#         column_master_id = insert(
#             con, screen_table.name, {"screen_cd": "column_master", "screen_name": "列管理"}
#         ).lastrowid
#
#         ## 画面管理用の設定
...
#     where screen_cd = "car_list"
#     ORDER BY column.view_order
#     """
#     print(sql)
#     print("---")
...
# with connect() as con:
#     # car_list_id = insert(
#     #     con, screen_table, {"screen_cd": "car_list", "screen_name": "カーリスト"}
#     # ).lastrowid
#     car_list_id = con.execute("SELECT screen_id FROM screen WHERE screen_cd = 'car_list'").fetchone()["screen_id"]
#
#     for i, col in enumerate(car_table.cols):
#         insert(
#             con,
#             column_table,
#             {
#                 "screen_id": car_list_id,
#                 "view_order": i,
#                 "column_cd": col.code,
#                 "column_name": col.code,
#                 "type": col.type_,
...
with connect() as con:
    ...
    con.execute("DELETE FROM column WHERE screen_id = (SELECT screen_id FROM screen WHERE screen_cd = 'class_master')")
    con.execute("DELETE FROM column WHERE screen_id = (SELECT screen_id FROM screen WHERE screen_cd = 'class_dtl_master')")
...
with connect() as con:
    make_table(class_table, "class_master", "区分値管理")
    make_table(class_dtl_table, "class_dtl_master", "区分値明細管理")

    make_class_id = insert(con, class_table.name, {'class_cd': "make", 'class_name': "make" }).lastrowid
    for v in ["Tesla", "Ford", "Toyota"]:
        insert(con, class_dtl_table.name, {'class_id': make_class_id, 'class_dtl_cd': v, 'class_dtl_name': v})

with connect() as con:
    con.execute("""UPDATE column SET required = 0 """)
